...
# Sitemap entries under these prefixes are generated from what is on disk. Any
# existing ones are discarded and rebuilt on every run; everything else in
# sitemap.xml is hand-written and only has its <lastmod> stamped.
GENERATED_PREFIXES = (SITE + BLOG_DIR + "/", SITE + "assets/papers/")
...
def paper_entries():
    """One sitemap entry per paper or slide deck under assets/papers/.

    These are content, not attachments. A conference paper PDF ranks on its own
    title, and for an academic site it is often the first thing a search finds.
    Generated by glob so that adding a paper never means editing sitemap.xml.
    """
    entries = []
    for path in sorted(glob.glob(os.path.join("assets", "papers", "*", "*.pdf"))):
        date = git_date(path, "%cs")
        print(f"{path} last changed: {date}")
        entries.append({"url": site_url(path), "lastmod": date, "priority": "0.5"})
    if not entries:
        fail("no PDFs found under assets/papers/ - has the layout changed?")
    return entries


def add_generated_urls(xml_text, entries):
    """Replace the generated <url> entries with freshly built ones.

    Any entry already under one of GENERATED_PREFIXES is dropped before the new
    ones are appended, which keeps the script idempotent: running it locally and
    committing what it produced does not wedge the next run the way appending
    blindly would. The hand-written entries, which have no shared prefix, are
    matched by <loc> elsewhere and left in place here.
    """
    for prefix in GENERATED_PREFIXES:
        xml_text = re.sub(
            r"[^\S\n]*<url>\s*<loc>" + re.escape(prefix)
            + r"[^<]*</loc>.*?</url>\n?",
            "",
            xml_text,
            flags=re.S,
...
def check_sitemap(xml_text):
    """Validate the sitemap against what the protocol actually requires.

    This is submitted to Search Console by hand, and a sitemap that parses but
    is malformed fails there rather than here, days later and without saying
    which entry was wrong. Cheaper to be strict at build time.
    """
    doc = xml.dom.minidom.parseString(xml_text)
    urls = doc.getElementsByTagName("url")
    if not urls:
        fail("sitemap.xml has no <url> entries")
    if len(urls) > 50_000 or len(xml_text.encode()) > 50 * 1024 * 1024:
        fail("sitemap.xml exceeds the protocol's 50,000 URL / 50 MB limit")

    seen = set()
    for u in urls:
        locs = u.getElementsByTagName("loc")
        if len(locs) != 1:
            fail(f"a <url> has {len(locs)} <loc> children; exactly 1 is required")
        loc = locs[0].firstChild.nodeValue.strip()
        if not loc.startswith(SITE):
            fail(f"{loc} is not an absolute URL under {SITE}. Sitemap entries "
                 f"must share the scheme and host of the sitemap itself.")
        if loc in seen:
            fail(f"{loc} appears twice in sitemap.xml")
        seen.add(loc)
        for mod in u.getElementsByTagName("lastmod"):
            value = mod.firstChild.nodeValue.strip()
            if not re.match(r"^\d{4}-\d{2}-\d{2}(T.*)?$", value):
                fail(f"{loc}: lastmod={value!r} is not a W3C datetime")
    print(f"sitemap.xml: {len(urls)} URLs, all absolute, unique and dated")
...
def main():
    ...
    xml_text = add_generated_urls(xml_text, blog_entries + paper_entries())
